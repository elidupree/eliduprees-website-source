...
stimulation_list = [
  ...
  simple_item ("spray me with water", 0, 3, 1, 5, hours, hours, "TODO description", {"not_when_tied": True}),
  simple_item ("dunk me in water", 1, 2, 1, 4, hours, half_hour, "TODO description", {"not_when_tied": True}),
...
impractical_list = [
  ...
  simplest_item ("literal magic", "There are so many sexy things you could do with magic. Shapeshifting… Mind control… Telekinesis… There are way too many for me to list them all.", {"control": "pretend to use magical mind control to make me", "possible_tied": True}),
...
function button (text, generators, parameters) {
  return $("<button>").css("display", "block").text ("Pick "+text).click (function() {
    ...
    display.text(result).css("height", ""+(2+number*10/7)+"em");
  });
}
