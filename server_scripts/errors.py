  ...
  def __init__(self, error_message):
    ...
    try:
      with open("/home/private/logs/all_website_errors", "a") as error_log:
        error_log.write(datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")+": "+os.environ["SCRIPT_NAME"]+username_string+": "+error_message+"\n")
    except IOError:
      error_message = error_message + "; additional error while trying to log the error in a file"
    self.error_message = error_message

def UncaughtError():
  error_message = "Unexpected error in "+os.environ["SCRIPT_NAME"]+": "+traceback.format_exc()
